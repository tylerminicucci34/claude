...
| **Research** | Live quote + 52-week range, **Marrow Quality Rating** (Growth / Business Quality / Financial Health / Valuation, each 0–100 with a composite), key TTM stats, Wall Street analyst consensus, EPS-vs-estimate history, company news, and a streaming **AI analyst chat** that knows all the data on screen | Finnhub + Anthropic API (your keys) |
| **Smart Money** | Superinvestor 13F holdings (Buffett, Ackman, Burry), insider Form 4 activity with parsed buy/sell summaries, and the latest House congress-trade reports (PTRs) linking to official PDFs | SEC EDGAR + House Clerk, refreshed weekly by GitHub Actions |
